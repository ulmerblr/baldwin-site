...
npm run check:images   # fails while any image slot is not real photography
npm run images:samples # regenerate the sample placeholder images
```

> **Both `check:tk` and `check:images` must pass before the DNS cutover.** They
> guard the two categories of deliberate blank on this site: facts nobody has
> supplied yet, and photography nobody has shot yet.
...
says where a slot stands. Nothing else decides it — components, the build banner and
`check:images` all read it through `resolveSlot()`, so "is this slot done?" has one
answer in one place.

| State | Renders | Counts as done |
| --- | --- | --- |
| `empty` | the neutral "photograph to come" treatment | no |
| `sample` | a generated stand-in, marked `SAMPLE — REPLACE` | **no** |
| `real` | the actual photograph | yes |

**Every slot is currently `sample`.** Samples are scaffolding: they make layout,
cropping and overlay contrast reviewable before any photography exists. A sample is
never a filled slot — `data-slot-filled` stays `"false"`, and `check:images` fails
while any remain.

Two consequences worth knowing:

- A sample's `alt` is forced empty and the image is hidden from assistive tech.
  `chris-portrait` is configured as "Chris Baldwin, founder of Baldwin Insurance
  Agency" — announcing that over a magenta test card would tell a screen reader user
  a photograph of a real person is present when it is not. The configured `alt` stays
  in the config, ready for the real file.
- Samples are generated by `npm run images:samples` into `public/images/samples/`.
  Their paths are *derived from the slot name*, never configured, so a sample cannot
  be typed into `src` and quietly pass as supplied artwork.

### Putting a real photo in

1. Save the file under `public/images/`.
2. In `src/content/images.ts`, set that slot's `state` to `'real'`, set `src`, and
   write `alt`.
...
The overlay **samples** are banded in sixteen narrow vertical stripes running from
pure white to near-black, and their own label sits low and narrow rather than across
the middle. Both choices are deliberate: narrow stripes mean even a short headline
crosses several, and a label through the vertical centre would sit exactly where
overlay headlines do and mask the bands underneath. Either mistake produces a
passing contrast number that proves nothing.

That stress test earned its keep immediately — it caught the hero and page-header
eyebrows, which were `gold-bright` small text sitting straight on the scrim. Against
the old flat-dark placeholder they measured 6.91:1; against a white band they are
2.56:1. Holding them to AA through the scrim alone would need a 0.88 floor, dark
enough to bury any real photograph, so the eyebrow pills were given their own opaque
ink ground instead — photo-independent at 6.91:1, with the scrim left light enough
for the image to read.
